...
var WSDL_PATH = "http://track.thailandpost.co.th/TTSPSW/track.asmx?WSDL" || "track.wsdl";
var USER_AGENT = "TTPTracker";
...
function TrackService(options) {
	...
	this.client = null;
	this.defaultArgs = {
		user: opts.user || "ca_sp_ios",
		password: opts.password || "520tXllm2MXZIGWjL/wchA==",
		lang: opts.lang.toLowerCase() || "en",
		deviceType: opts.deviceType || "IOS"
	};
}
...
TrackService.prototype.getNearbyLocations = function getNearbyLocations(loc, point, callback) {
	var args = {
		user: this.defaultArgs.user,
		password: this.defaultArgs.password,
		deviceType: this.defaultArgs.deviceType,
	...
	this.client.GetLocationClosest(args, function(err, res) {
		...
		callback(null, res.GetLocationClosestResult);
  	});
};
